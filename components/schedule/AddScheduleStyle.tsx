...
export const ASInput = styled.TextInput.attrs<StyledProps>(props => ({
    placeholderTextColor: props.$colors?.borderColor,
}))<StyledProps>`
    width: 100%;
    border-radius: 8px;
    padding: 12px 15px;
...
export const ASPickerWrap = styled.View<StyledProps>`
    width: 100%;
    border: 1px solid ${props => props.$colors?.borderColor || '#e0e0e0'};
    border-radius: 8px;
    background-color: ${props => props.$colors?.borderColor || 'lavender'};
    justify-content: center;
    margin-bottom: 20px;
...
export const ASSelectedTag = styled.View<StyledProps & { color: string }>`
    flex-direction: row;
    align-items: center;
    background-color: ${(props) => props.color || props.$colors?.background || 'gray'};
    border-radius: 30px;
    padding: 10px 25px;
    margin-bottom: 100px;
    margin-top: 20px;
...
export const ASButtonWrap = styled.View<StyledProps>`
    ...
    flex-direction: row;
    justify-content: space-between;
    border: 2px solid ${props => props.$colors?.borderColor || '#f0f0f0'};
    background-color: ${props => props.$colors?.background || '#ffffff'};
`
...
export const ASSaveButton = styled.TouchableOpacity<StyledProps>`
    ...
    border-right-color: ${props => props.$colors?.borderColor || '#f0f0f0'};
`;
...
export const ModalContainer = styled.View<StyledProps>`
    width: 90%;
    background-color: ${props => props.$colors?.borderColor || 'white'};
    border-radius: 15px;
    padding: 25px;
    align-items: stretch;
`;
...
export const ActionButton = styled.TouchableOpacity<StyledProps & { primary?: boolean }>`
    ...
    align-items: center;
    border-radius: 8px;
    background-color: ${props => (props.primary ? (props.$colors?.pointColors.purple || '#9970FF') : (props.$colors?.borderColor || '#f0f0f0'))};
    margin: 0 5px;
`;

export const ButtonText = styled.Text<StyledProps & { primary?: boolean }>`
    color: ${props => (props.primary ? (props.$colors?.pointColors.white || 'white') : (props.$colors?.text))};
    font-size: 16px;
    font-weight: bold;
`;
