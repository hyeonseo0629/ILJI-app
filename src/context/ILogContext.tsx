...
import { ILog, RawILog } from '@/src/types/ilog';
...
interface ILogContextType {
    ...
    createILog: (data: { request: Omit<ILog, 'id' | 'userId' | 'createdAt'>; images?: File[] }) => Promise<ILog | null>;
...
export function ILogProvider({ children }: ILogProviderProps) {
    ...
    const formatRawILog = useCallback((raw: RawILog): ILog => ({
        ...raw,
        iLogDate: new Date(raw.iLogDate),
    ...
    const createILog = useCallback(async (data: { request: Omit<ILog, 'id' | 'userId' | 'createdAt'>; images?: File[] }): Promise<ILog | null> => {
        ...
        try {
            ...
            setIlogs(prev => [newLog, ...prev].sort((a, b) => b.iLogDate.getTime() - a.iLogDate.getTime()));
            return newLog;
        } catch (err) {
